...
class QRSDetector(object):
    ...
    def __init__(self, ecg_data, verbose=False, log_data=False, plot_data=False, show_plot=False):
        """
    ...
    def bandpass_filter(self, data, lowcut, highcut, signal_freq, filter_order):
        ...
        b, a = butter(filter_order, fs =signal_freq, Wn=[lowcut, highcut], btype="bandpass")
        y = lfilter(b, a, data)
        return y
...
if __name__ == "__main__":
    ...
    i = 2
    data = np.loadtxt(cwd+'/ecg_data_1.csv', skiprows=1, delimiter=',')[800*i:800*(i+1)+100,:]
    qrs_detector = QRSDetector(data)
    print("slut")
    
    peakIdx = qrs_detector.qrs_peaks_indices
    peakval = qrs_detector.qrs_peak_value
    
    #plot data signal and set line for every peak idx
    plt.plot(data[:,1])
    plt.plot(peakIdx,data[peakIdx,1],color="black")
    plt.show() 
    ...
    print("antal rr: "+str(len(rr_interval)))
    time_domain_features = get_time_domain_features(rr_interval)
    print("mean_hr: " + str(time_domain_features['mean_hr']))
    NonLineardomainfeatures = get_csi_cvi_features(rr_interval)
    print(NonLineardomainfeatures)
    v1 = get_time_domain_features(rr_interval)
    v2 = get_geometrical_features(rr_interval)
    v3 = get_frequency_domain_features(rr_interval)
    v4 = get_csi_cvi_features(rr_interval)
    v5 = get_poincare_plot_features(rr_interval)
    v6 = get_sampen(rr_interval)
